...
def etl_pipeline(source_name, input_path, output_path):
    try:
        ...
        print(f"[{source_name}] Thread: {threading.current_thread().name}, PID: {os.getpid()}, Core ID: {core_id}")
        print(f"[{source_name}] Memory Usage: {round(psutil.Process().memory_info().rss / (1024 ** 2), 2)} MB")
        print(f"[{source_name}] Per-Core CPU Usage: {psutil.cpu_percent(interval=1, percpu=True)}%")
        
        # print(f"[{source_name}] Thread: {threading.current_thread().name}, PID: {os.getpid()}, Core ID: {psutil.Process().cpu_num()}")
        # print(f"[{source_name}] Memory Usage: {round(psutil.Process().memory_info().rss / (1024 ** 2), 2)} MB")
        ...
        print(f"[{source_name}] Finished in {elapsed:.2f} seconds\n")
    except Exception as e:
...
def time_threshold():
    now = datetime.now()
    threshold_time = now.replace(hour=1, minute=7, second=0, microsecond=0)
